...
function parseSearchPage(html) {
  ...
  $('article.item').each((_, el) => {
    ...
    if (downloadPage && !downloadPage.startsWith('http')) downloadPage = 'https://mp3.tubidy.com' + downloadPage;
    ...
    if (title && downloadPage) results.push({ title, download_page: downloadPage, thumbnail, duration: durationText, duration_iso: durationIso });
  });
  return results;
}
...
function hasNextPage(html, currentPage) {
  ...
  let found = false;
  $('a[href]').each((_, el) => {
    if (($(el).attr('href') || '').includes(`page=${currentPage + 1}`)) { found = true; return false; }
  });
